...
for bam in *.name_sorted.bam; do
  echo Processing "${bam}"
  samtools view -@ 6 -h "${bam/name_sorted/hic_info}" \
    | "${filter_reads}" --min_inward 1000 --max_ditag 1000 - \
    | samtools view -@ 6 -b > "${bam/name_sorted/filtered}"
  samtools sort -@ 3 "${bam/name_sorted/filtered}" > "${bam/name_sorted/filtered-sort}"
  samtools index -@ 3 "${bam/name_sorted/filtered-sort}"
done
