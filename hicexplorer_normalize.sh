...
mkdir -p "${dir}"/correlation_plots "${dir}"/diagnostic_plots \
         "${dir}"/counts_vs_dist "${dir}"/"${binsize}"
...
for group in "${groups[@]}"; do
    ...
    for matrix in "${dir}"/"${binsize}"/"${group}"*-norm*(_sum).h5; do
        ...
        hicCorrectMatrix correct --matrix "${matrix}" --correctionMethod ICE \
            --iterNum 1000 --filterThreshold "${lower_threshold}" 5 \
            --outFileName "${matrix%.h5}"_iced.h5
...
for transform in count obs_exp; do
    for track in "${tracks[@]}"; do
        ...
        hicPlotTADs --tracks "${plot_track}" --region ${plot_range} \
                --outFileName "${plotname}" --dpi 50 \
            2> "${dir}"/"${binsize}"/tad_plots/${track##*/}_plot_log_temp.txt
