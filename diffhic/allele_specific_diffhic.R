...
count.keep = ave.ab >= aveLogCPM(1, lib.size=mean(data$totals))
summary(count.keep)
...
Sample <- gl(2,2,length=8)
Cell <- factor(c("HB2_WT","HB2_WT", "HB2_WT","HB2_WT", "HB2_CL4","HB2_CL4", "HB2_CL4","HB2_CL4"),
               levels = c("HB2_WT", "HB2_CL4"))
Treat <- factor(c("G1", "G2", "G1", "G2", "G1", "G2", "G1", "G2"), 
                levels = c("G1","G2"))
data.frame(Cell,Sample,Treat)
design <- model.matrix(~Cell+Cell:Sample+Cell:Treat)
...
result <- glmQLFTest(fit, coef="CellHB2_WT:TreatG2")
topTags(result)
