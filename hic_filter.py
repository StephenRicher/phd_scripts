...
def main():
    ...
    digest_filename="Homo_sapiens.GRCh38.dna.primary_assembly_MboI.bed"
    ...
    try:
        with open("HB2_WT_1.fix.pair.sam") as sam_file:
            for line in sam_file:
                if line.startswith("@"):
                    ...
                    read1_fragment = get_fragment(read1, d)
                    read2_fragment = get_fragment(read2, d)
        ...
        print("Valid pairs", valid_cis + valid_trans)
        print("Trans", valid_trans)
        print("Invalid pairs", total_pairs - (valid_cis + valid_trans))
        print("Self circle", self_circle)
        print("Dangling end", dangling_end)
        print("Same strand error", same_strand_error)
        print("Religation", religation)
...
if __name__ == "__main__":
    ...
    #mysamfile = open("/media/stephen/Data/hic_analysis/data/test.sam")
    #all_left_pysam = []
    #all_left_my = []
    #for read in samfile.fetch():
    #    all_left_pysam.append(read.get_reference_length()[-1])
    #for read in mysamfile:
    #    if not read.startswith("@"):
    #        myread = sam(read.split())
    #        all_left_my.append(myread.right_pos)
    #for i in range(len(all_left_pysam)):
    #    if all_left_pysam[i] + 1 != all_left_my[i]:
    #        print(all_left_pysam[i], all_left_my[i])
